...
nets = dict(
    ...
    execoin=math.Object(
      ...
      rotocoin=math.Object(
        ...
        RPC_PORT=7783,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'planetdollaraddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        ...
        SYMBOL='PDR',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'],
            'planetdollar') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/planetdollar/') if platform.system() == 'Darwin' else os.path.expanduser('~/.planetdollar'), 'planetdollar.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://cryptexplorer.com/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://cryptexplorer.com/address/',
        TX_EXPLORER_URL_PREFIX='http://cryptexplorer.com/tx/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1),
